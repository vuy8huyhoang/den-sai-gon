// components/flashsale/FlashSale.tsx
            ...
            <div className="relative mx-auto rounded-[12px] bg-[linear-gradient(180deg,#ffd6d6_0%,#ffe1e1_35%,#ffd7d7_60%,#ffcaca_100%)] shadow-[0_6px_18px_rgba(0,0,0,0.08)]">
                <div className="relative rounded-[14px] m-[10px] bg-white/0">
                    ...
                    <div className="pointer-events-none select-none absolute left-1/2 -translate-x-1/2 -top-7 z-10">
                        <div className="relative w-[min(620px,calc(100vw-40px))] h-[56px]">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center px-4">
                                <h2 className="text-white font-extrabold tracking-wide text-[min(22px,5vw)] drop-shadow-[0_2px_0_rgba(0,0,0,0.2)] text-center leading-tight">
                                    {title}
                                </h2>
                            </div>
                            <div className="absolute md:-left-5 -left-4 top-[10px] w-5 h-5 rotate-45 bg-[#d42727]" />
                            <div className="absolute md:-right-5 -right-4 top-[10px] w-5 h-5 -rotate-45 bg-[#d42727]" />
                        </div>
                    </div>

                    {/* Tabs & countdown (nguyên vẹn) */}
                    <div className="pt-10 px-3 sm:px-6">
                        <div className="relative flex flex-col sm:flex-row items-center gap-2 sm:gap-4">
                            <GiftIcon className="hidden sm:block" />
                            <div className="flex items-center gap-2 sm:gap-4 flex-wrap justify-center sm:justify-start">
                                <Tab label={windowLabel} active />
                                <Tab label={nextWindowLabel} />
                            </div>
                            <div className="flex-1" />
                            <div className="flex items-center gap-2 sm:gap-3 text-[#b71d1d] mt-2 sm:mt-0">
                                <span className="font-bold text-[14px] sm:text-[16px]">BẮT ĐẦU SAU:</span>
                                <CountdownBox value={hh} />
                                <span className="font-bold text-[14px] sm:text-[16px]">:</span>
                                <CountdownBox value={mm} />
                                <span className="font-bold text-[14px] sm:text-[16px]">:</span>
                                <CountdownBox value={ss} />
                            </div>
                            <GiftIconRight className="hidden sm:block" />
                        </div>
                    </div>

                    {/* Slider sản phẩm (giữ layout, thêm drag handlers) */}
                    <div className="relative px-3 sm:px-6 pb-6 pt-2">
                        <button
                            onClick={() => scrollBy(-1)}
                            aria-label="prev"
                            className="absolute left-1 sm:left-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white shadow-md border hover:bg-slate-50"
                        >
                        ...
                        <button
                            onClick={() => scrollBy(1)}
                            aria-label="next"
                            className="absolute right-1 sm:right-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white shadow-md border hover:bg-slate-50"
                        >
                        ...
                        <div className="relative overflow-hidden bg-white">
                            ...
                            >
                                ...
                                <div
                                    className={[
                                        "grid grid-flow-col auto-cols-[280px] sm:auto-cols-[320px] md:auto-cols-[300px] gap-3 sm:gap-4 px-1 sm:px-2",
                                ...
                                >
                                    {products.map((p) => {
                                        ...
                                        const discount =
                                            p.compareAt && p.compareAt > min ? Math.round((1 - min / p.compareAt) * 100) : undefined
                                        ...
                                        return (
                                            <ItemCard
                                                ...
                                                featureKeys={["size", "led3mau", "dong", "inox201", "phaleK9"]}
...
function Tab({ label, active = false }: { label: string; active?: boolean }) {
    return (
        <button
            className={[
                "h-[40px] sm:h-[44px] px-3 sm:px-5 rounded-[20px] sm:rounded-[24px] text-[12px] sm:text-[14px] font-semibold",
...
function CountdownBox({ value }: { value: string }) {
    return (
        <div className="w-[36px] h-[28px] sm:w-[40px] sm:h-[32px] rounded-[6px] bg-white text-[#b71d1d] flex items-center justify-center font-extrabold tracking-wider shadow-[0_2px_0_rgba(0,0,0,0.1)] text-[14px] sm:text-[16px]">
            {value}
        </div>
    )
}
...
export const DEMO_PRODUCTS: FSProduct[] = [
    {
        id: 1,
        title: "Đèn chùm pha lê 12 tay cao cấp",
        image: "/den-sai-gon/anh-sp/den-chum.png",
        priceMin: 11241000,
        priceMax: 13990000,
        compareAt: 14990000,
        sold: 5,
    ...
    {
        id: 2,
        title: "Đèn chùm hiện đại 10 tay",
        image: "/den-sai-gon/anh-sp/den-chum.png",
        priceMin: 8900000,
        priceMax: 12500000,
        compareAt: 13900000,
    ...
    {
        id: 3,
        title: "Đèn chùm phong cách Bắc Âu",
        image: "/den-sai-gon/anh-sp/den-chum.png",
        priceMin: 4990000,
        priceMax: 9500000,
        compareAt: 10500000,
