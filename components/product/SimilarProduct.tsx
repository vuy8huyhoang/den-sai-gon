// ✅ Drop-in replacement for your two files with better drag UX
// - Prevent image/text selection while dragging
// - Block accidental clicks on links/buttons when the user is dragging
// - Disable snap during drag for smoother feel
// - Wheel → horizontal scroll, arrows retained

// =============================================
// components/product/SimilarProduct.tsx
// =============================================
'use client'
...
function RowSlider({ products, cardWidth = 276 }: { products: Product[]; cardWidth?: number }) {
    ...
    const end = () => {
        ...
        // release after a frame so click-capture can see dragged flag
        requestAnimationFrame(() => setDragging(false))
    }

    // Prevent default native drag (images/links ghost)
    const onDragStartCapture: React.DragEventHandler<HTMLDivElement> = (e) => {
        if (dragging) e.preventDefault()
    }

    // Swallow clicks if it was a drag
        ...
        <div className="relative">
            <div
                ...
                className={[
                    ...
                    // Touch devices: allow vertical scroll of page
                    'touch-pan-y'
                ].join(' ')}
            >
                {products.map((p) => (
...
export default function SimilarProduct({
    ...
    return (
        <section className={className}>
            ...
            <RowSlider products={products} cardWidth={cardWidth} />
        </section>
    )
}
